...
ssh -i ~/.ssh/id_rsa_ci -p $PORT "$USER"@"$HOST" drush -u 1 --root=$PATH_DEST sql-dump > $PATH_SOURCE/mysql/db.sql
echo "Restore from dump $PATH_SOURCE/mysql/db.sql"
drush -u 1 --root=$PATH_SOURCE sql-drop -y
drush -u 1 --root=$PATH_SOURCE sql-cli < $PATH_SOURCE/mysql/db.sql
echo "Pull code"
cd $PATH_SOURCE
git reset --hard
git checkout master
