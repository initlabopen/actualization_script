...
rsync -av -e "ssh -p $PORT -i $HOME/.ssh/id_rsa_ci" "$USER"@"$HOST":"$PATH_DEST/sites/default/files/" "$PATH_SOURCE/sites/default/files/" --exclude 'settings.php'
echo "Create dump to $PATH_SOURCE/mysql/db.sql"
mkdir -p $PATH_SOURCE/mysql
ssh -i $HOME/.ssh/id_rsa_ci -p $PORT "$USER"@"$HOST" drush --root=$PATH_DEST sql-dump > $PATH_SOURCE/mysql/db.sql
